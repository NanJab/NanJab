# 👋 안녕하세요, 나성채입니다!  

<div align="center">
  <img src="나성채 클라이언트 프로그래머.png" width="70%">
</div>

## 🚀 저는 누구인가요?  
- **클라이언트 게임 프로그래머**를 꿈꾸는 개발자  
- Unity + C#으로 RPG 게임 제작 중  
- 문제 해결을 위한 **끈질긴 집요함**이 장점  

## 🎮 프로젝트  
| 프로젝트 | 설명 |  
|----------|------|  
| [RPG Adventure](https://plum-tarsier-413.notion.site/169e3fef2695809fb075d53fc3534c1a) | 장비/퀘스트 시스템 구현 |  
| [테트리스](https://plum-tarsier-413.notion.site/169e3fef2695809fb075d53fc3534c1a) | DirectX로 제작한 클래식 게임 |  

## 📚 기술 스택  

### 프로그래밍 언어  
![C](https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=white)  
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)  
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp)  

### 게임 엔진  
![Unity](https://img.shields.io/badge/Unity-FFFFFF?style=flat&logo=unity)  

### **기술 역량**  
| 분야 | 경험 내용 |
|------|----------|
| **게임 시스템** | 인벤토리, 퀘스트, 스킬 시스템 설계 |
| **네트워크** | Mirror 기반 멀티플레이어 동기화 구현 |
| **최적화** | 객체 풀링 |

---

## 🎮 프로젝트 경험  

### 1. **RPG 게임 (Unity/C#)**  
- **인벤토리 시스템**  
  - 장비 희귀도별 정렬 알고리즘 구현  
  - 드래그 앤 드롭으로 아이템 장착/해제  
- **퀘스트 시스템**  
  - NPC 대화 → 목표 추적 → 보상 지급 자동화  
- **스킬 시스템**
  - 쿨타임 기능 구현  

### 2. **테트리스 (C++/DirectX)**  
- **렌더링**  
  - Direct2D로 블록 색상/형태 구현  
  - 비트맵 기반 충돌 감지 로직 개발  
- **게임 로직**  
  - 블록 회전/이동 물리 시스템 설계

---

## 📂 주요 성과  
- RPG 게임: **인벤토리 시스템 개발로 아이템 관리 편의성 50% 향상**  
- 테트리스: **회전/충돌 판정 로직 직접 구현 → 공간 감각 및 수학적 사고력 향상**

---

## 📬 연락처  
✉️ **Email**: nsc6673@naver.com  
🌎 **Blog**: [기술 블로그](https://jabchae4919.tistory.com/)  
🔗 **포트폴리오**: https://plum-tarsier-413.notion.site/169e3fef2695809fb075d53fc3534c1a

## 나성채의 GitHub Stats 
![나성채의 GitHub Stats](https://github-readme-stats.vercel.app/api?username=NanJab&show_icons=true&theme=radical)

🔨 **현재 작업 중**:  
- [x] 인벤토리 UI 및 드래그&드랍
- [ ] 인벤토리 마우스 휠 슬롯 강조조
